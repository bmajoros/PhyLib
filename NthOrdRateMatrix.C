/****************************************************************
 NthOrdRateMatrix.H
 william.majoros@duke.edu

 This is open-source software, governed by the Gnu General Public License (GPL) version 3 
 (see www.opensource.org).
 ****************************************************************/
#include "NthOrdRateMatrix.H"
#include "BOOM/GSL/LabeledMatrixLoader.H"
#include "BOOM/GSL/Vector.H"
#include "BOOM/DnaAlphabet.H"
#include "BOOM/AminoAlphabet.H"
#include "BOOM/PureDnaAlphabet.H"
#include "BOOM/DnaDashDotAlphabet.H"
#include "BOOM/GapPatternAlphabet.H"
#include "BOOM/GapPatternAlphaMap.H"
#include "BOOM/Random.H"
#include "BOOM/Sequence.H"
#include "BOOM/GSL/GaussianDistribution.H"
#include "JukesCantor.H"
#include "FEL.H"
#include "Kimura2Param.H"
#include "HKY.H"
#include "REV.H"
#include <fstream>
#include "PeriodicRateMatrix.H"
#include "NmerRateMatrix.H"
using namespace BOOM;


ostream &operator<<(ostream &os,NthOrdMatrixType t)
{
  switch(t) {
  case NOMT_COND: os<<"COND"; break;
  case NOMT_NMER: os<<"NMER"; break;
  case NOMT_NMER_RAW: os <<"NMER_RAW"; break;
  default: throw "unrecognized NthOrdMatrixType";
  }
}



istream &operator>>(istream &is,NthOrdMatrixType &t)
{
  String line;
  is>>line;
  line.trimWhitespace();
  if(line=="COND") t=NOMT_COND;
  else if(line=="NMER") t=NOMT_NMER;
  else if(line=="NMER_RAW") t=NOMT_NMER_RAW;
  else throw line+" : unrecognized NthOrdMatrixType";
  return is;
}



BOOM::Array1D<double> NthOrdRateMatrix::omitted;

/*
  NOTE: In dual models (those for which we model both parental and child
  contexts), since the parent & child context lengths are assumed equal,
  there can only be even orders.  However, we keep the odd orders anyway,
  because they can be used to generate successively higher-order models 
  via Gaussian perturbation (for simulation runs).
 */

NthOrdRateMatrix::NthOrdRateMatrix(int order,MolecularSequenceType seqType,
				   RateMatrixType matrixType,bool dual)
  : order(order), 
    seqType(seqType), 
    matrixType(matrixType),
    lowerOrderModel(NULL),
    alphabet(seqType==DNA ? 
	     (matrixType==MT_GAP ? 
	        (Alphabet&) GapPatternAlphabet::global() :
	        (Alphabet&) PureDnaAlphabet::global()) 
	     : 
	     (Alphabet&) AminoAlphabet::global()),
    dual(dual),
    parmToContextMap(NULL)
{
  // ctor

  if(seqType==DNA)
    if(matrixType==MT_GAP)
      alphabetMap=GapPatternAlphaMap::global();
    else
      alphabetMap=DropGapMapping(DnaDashDotAlphabet::global(),
				 PureDnaAlphabet::global());
  else
    alphabetMap=AlphabetIdentityMap(alphabet);

  int trueOrder=order;
  if(dual && order%2>0) trueOrder=order/2+1; // ODD ORDER IN DUAL MODEL
  int numMatrices=pow(float(alphabet.size()),float(trueOrder));
  matrices.resize(numMatrices);
  matrices.setAllTo(NULL);

  if(order>0) 
      lowerOrderModel=new NthOrdRateMatrix(order-1,seqType,matrixType,dual);
}



NthOrdRateMatrix::~NthOrdRateMatrix()
{
  int n=matrices.size();
  for(int i=0 ; i<n ; ++i)
    delete matrices[i];

  delete lowerOrderModel;
}



void NthOrdRateMatrix::randomize(double noiseSigma,
                                 BOOM::Array1D<double> equilibriumFreqs) {

    // PRECONDITION: the model is NOT a dual model

  eqFreqs=equilibriumFreqs;
  GSL::GaussianDistribution noiseModel(0,noiseSigma);
  if(!lowerOrderModel)
    matrices[0]=RateMatrix::random(seqType,matrixType,equilibriumFreqs);
  else {
    lowerOrderModel->randomize(noiseSigma,equilibriumFreqs);
    Array1D<RateMatrix*> &lowerOrderMatrices=lowerOrderModel->matrices;
    Sequence nmer;
    int numMatrices=matrices.size();
    for(int i=0 ; i<numMatrices ; ++i) {
      nmer.fromInt(i,order,alphabetMap);
      int lowerOrderIndex=nmer.asInt(alphabetMap,1,order-1);
      RateMatrix *M=lowerOrderMatrices[lowerOrderIndex];
      RateMatrix *newM=M->clone();
      newM->addNoise(noiseModel);
      matrices[i]=newM;
    }
  }
  rescale();
}



void NthOrdRateMatrix::randomizeDual(double noiseSigma,
                                     BOOM::Array1D<double> equilibriumFreqs) 
{
  eqFreqs=equilibriumFreqs;
  Sequence nmer, parentNmer, childNmer;
  GSL::GaussianDistribution noiseModel(0,noiseSigma);
  
  if(!lowerOrderModel) // ZEROTH ORDER
    matrices[0]=RateMatrix::random(seqType,matrixType,equilibriumFreqs);
  
  else if(order%2>0) { // ODD ORDER => PARENT CONTEXT ONLY
    lowerOrderModel->randomizeDual(noiseSigma,equilibriumFreqs);
    NthOrdRateMatrix *lowerOddOrderModel=lowerOrderModel;
    if(lowerOrderModel->lowerOrderModel)
      lowerOddOrderModel=lowerOrderModel->lowerOrderModel;
    Array1D<RateMatrix*> &lowerOrderMatrices=lowerOddOrderModel->matrices;
    int trueOrder=order/2+1;
    int numNmers=pow((float)alphabetMap.getRangeSize(),(float)trueOrder);
    for(int nmerCode=0 ; nmerCode<numNmers ; ++nmerCode) {
      parentNmer.fromInt(nmerCode,trueOrder,alphabetMap);
      int suffixCode=parentNmer.asInt(alphabetMap,1,trueOrder-1);
      RateMatrix *baseModel=lowerOrderMatrices[suffixCode];
      RateMatrix *newModel=baseModel->clone();
      newModel->addNoise(noiseModel);
      matrices[nmerCode]=newModel;
    }
  }
  
  else { // EVEN ORDER (GREATER THAN ZERO) => PARENT & CHILD CONTEXTS
    int halfOrder=order/2;
    lowerOrderModel->randomizeDual(noiseSigma,equilibriumFreqs);
    Array1D<RateMatrix*> &lowerOrderMatrices=lowerOrderModel->matrices;
    int numMatrices=matrices.size();
    for(int i=0 ; i<numMatrices ; ++i) {
      nmer.fromInt(i,order,alphabetMap);
      int parentNmerCode=nmer.asInt(alphabetMap,0,halfOrder);
      RateMatrix *baseModel=lowerOrderMatrices[parentNmerCode];
      RateMatrix *newM=baseModel->clone();
      newM->addNoise(noiseModel);
      matrices[i]=newM;
    }
  }

  rescale();
}



NthOrdRateMatrix *NthOrdRateMatrix::random(bool dual,
                                           double noiseSigma,
                                           MolecularSequenceType seqType,
                                           int order,
                                           RateMatrixType matrixType,
                                           BOOM::Array1D<double> 
                                           equilibriumFreqs=omitted) 
{
  if(matrixType==MT_NMER) {
    NmerRateMatrix *Q=new NmerRateMatrix(order+1);
    Q->eqFreqs=equilibriumFreqs;
    Q->randomize(noiseSigma,equilibriumFreqs);
    return Q;
  }
  else {
    NthOrdRateMatrix *Q=new NthOrdRateMatrix(order,seqType,matrixType,dual);
    Q->eqFreqs=equilibriumFreqs;
    if(dual)
      Q->randomizeDual(noiseSigma,equilibriumFreqs);
    else
      Q->randomize(noiseSigma,equilibriumFreqs);
    return Q;
  }
}



void NthOrdRateMatrix::save(const BOOM::String &filename)
{
  ofstream os(filename.c_str());
  save(os);
}



void NthOrdRateMatrix::save(ostream &os)
{
  os<<NOMT_COND<<endl; // NthOrdMatrixType
  os<<0<<endl; // 0=not periodic ("no")
  os<<(dual?1:0)<<endl;
  os<<seqType<<endl;
  os<<matrixType<<endl;
  os<<order<<endl;
  unsigned n=matrices.size();
  os<<n<<endl;
  RateMatrix *Q0=order>0 ? getLowerOrderModel(0)->matrices[0] : NULL;
  for(unsigned i=0 ; i<n ; ++i) {
    RateMatrix *Q=matrices[i];
    if(!Q) Q=Q0; // odd-order models usually are not allocated...
    Q->save(os);
  }
  
  if(lowerOrderModel)
    lowerOrderModel->save(os);
}



NthOrdRateMatrix *NthOrdRateMatrix::load(const BOOM::String &filename)
{
  ifstream is(filename.c_str());
  if(!is.good()) throw BOOM::String("Error opening file: ")+filename;
  return load(is);
}



NthOrdRateMatrix *NthOrdRateMatrix::load(istream &is)
{
  NthOrdMatrixType t;
  is>>t;
  if(t==NOMT_NMER) return NmerRateMatrix::load(is);
  if(t==NOMT_NMER_RAW) return NmerRateMatrix::loadRaw(is);

  int isPeriodic;
  is>>isPeriodic;
  if(isPeriodic) {
    PeriodicRateMatrix *M=
      new PeriodicRateMatrix(0,(MolecularSequenceType)0,(RateMatrixType)0,
			     false);
    M->load_noIndicator(is);
    return M;
  }

  int numMatrices;
  MolecularSequenceType seqType;
  RateMatrixType matrixType;
  int order, dualInt;
  is>>dualInt;
  bool dual=dualInt;
  is>>seqType>>matrixType>>order>>numMatrices;
  NthOrdRateMatrix *Q=new NthOrdRateMatrix(order,seqType,matrixType,dual);
  for(int i=0 ; i<numMatrices ; ++i)
    Q->matrices[i]=RateMatrix::load(is);

  if(order>0)
    Q->lowerOrderModel=load(is);

  return Q;
}



RateMatrixType NthOrdRateMatrix::getType()
{
  return matrixType;
}



int NthOrdRateMatrix::getOrder() const
{
  return order;
}



RateMatrix &NthOrdRateMatrix::lookup(const Sequence &context,int phase,
				     int begin,int len) {
  int L=len;
  if(L<0) L=context.getLength();
  if(L==order) {
    int index=context.asInt(alphabetMap,begin,len);
    return *matrices[index];
  }
  else if(L<order)
    return lowerOrderModel->lookup(context,phase,begin,len);
  else
    throw "Invalid context in NthOrdRateMatrix::lookup()";
}



int NthOrdRateMatrix::lookupIndex(const Sequence &parentContext,
                                  const Sequence &childContext) {
    // PRECONDITION: this is a DUAL model
    
    int L=parentContext.getLength()+childContext.getLength();
    if(L==order) {
        Sequence context=parentContext;
        context.append(childContext);
        return context.asInt(alphabetMap);
    }
    else {
        cout<<"parentContext:";
        parentContext.printOn(cout,alphabet);
        cout<<"\nchildContext:";
        childContext.printOn(cout,alphabet);
        cout<<endl;
        throw "Invalid context in NthOrdRateMatrix::lookupIndex()";
    }
}



RateMatrix &NthOrdRateMatrix::lookup(const Sequence &parentContext,
                                     const Sequence &childContext,
				     int phase) {
    Sequence context=parentContext;
    context.append(childContext);
    return lookup(context);
}



RateMatrix &NthOrdRateMatrix::lookup(unsigned contextCode,int contextLength)
{
  if(contextLength==order)
    return *matrices[contextCode];
  else 
    return lowerOrderModel->lookup(contextCode,contextLength);
}



NthOrdSubstMatrix *NthOrdRateMatrix::instantiate(double branchLength)
{
  NthOrdSubstMatrix *Pt=new NthOrdSubstMatrix(order,alphabetMap);
  instantiate(branchLength,*Pt);
  return Pt;
}



void NthOrdRateMatrix::instantiate(double branchLength,NthOrdSubstMatrix &Pt)
{
    unsigned numMatrices=matrices.size();
    for(unsigned i=0 ; i<numMatrices ; ++i) {
        RateMatrix *M=matrices[i];
        if(!M) continue;
        SubstitutionMatrix *matrix=M->instantiate(branchLength);
	matrix->convertToLogs();
        Pt.setMatrix(i,matrix);
    }
    if(lowerOrderModel) {
        Pt.setLowerOrderModel(lowerOrderModel->instantiate(branchLength));
    }
}



void NthOrdRateMatrix::init()
{
  int numMatrices=matrices.size();
  for(unsigned i=0 ; i<numMatrices ; ++i) {
    RateMatrix *M=matrices[i];
    if(M) M->init();
  }
  rescale();
}



int NthOrdRateMatrix::getNumRows() const
{
  RateMatrix *Q=matrices[0];
  if(!Q) throw "NULL matrix in NthOrdRateMatrix::getNumRows()";
  return Q->getNumRows();
}



AlphabetMap &NthOrdRateMatrix::getAlphabetMap()
{
  return alphabetMap;
}



const Alphabet &NthOrdRateMatrix::getAlphabet() const
{
  return alphabet;
}



int NthOrdRateMatrix::numParameters() const
{
  RateMatrix *Q=matrices[0];
  if(!Q) throw "NULL matrix in NthOrdRateMatrix::numParameters()";
  return Q->numParameters();
}



RateMatrixType NthOrdRateMatrix::getMatrixType()
{
  return matrixType;
}



ostream &operator<<(ostream &os,const NthOrdRateMatrix &Q)
{
  Q.printOn(os);
  return os;
}



void NthOrdRateMatrix::printOn(ostream &os) const
{
  unsigned n=matrices.size();
  Sequence context;
  for(unsigned i=0 ; i<n ; ++i) {
    context.fromInt(i,order,alphabetMap);
    os<<context<<endl;
    RateMatrix *Q=matrices[i];
    if(!Q) continue;
    os<<*Q<<endl;
  }
  if(lowerOrderModel)
    lowerOrderModel->printOn(os);
}



int NthOrdRateMatrix::getNumMatrices() const {
  return matrices.size();
}



void NthOrdRateMatrix::replaceIthMatrix(int i,RateMatrix *m)
{
  delete matrices[i];
  matrices[i]=m;
}



RateMatrix &NthOrdRateMatrix::getIthMatrix(int i,int phase) 
{
  return *matrices[i];
}



bool NthOrdRateMatrix::isPeriodic() 
{
  return false;
}



NthOrdRateMatrix *NthOrdRateMatrix::getLowerOrderModel(int order,int phase) {
    if(order<0 || order==this->order-1) return lowerOrderModel;
    return lowerOrderModel->getLowerOrderModel(order);
}



bool NthOrdRateMatrix::isDual() {
    return dual;
}



/*
  The following method changes the lower order models to make them
  consistent with the higher-order models.  It does this by averaging
  the higher-order RateMatrix's having the same lower-order context suffix.
 */
void NthOrdRateMatrix::averageLowerOrderModel(bool dual) {
    if(!lowerOrderModel) return;
    if(dual) {averageDual(); return;}
    Array1D<RateMatrix*> &lowerOrderMatrices=lowerOrderModel->matrices;
    int alphaRange=alphabetMap.getRangeSize();
    int numSuffixes=lowerOrderMatrices.size();
    for(int i=0 ; i<numSuffixes ; ++i) {
        Sequence suffix;
        suffix.fromInt(i,order-1,alphabetMap);
        Array1D<RateMatrix*> hiMs(alphaRange);
        for(Symbol y=0 ; y<alphaRange ; ++y) {
	  Symbol x=alphabetMap.unmap(y);//###?
            Sequence context;
            context.append(x);
            context.append(suffix);
            hiMs[y]=&lookup(context);
        }
        RateMatrix *&lowerOrderMatrix=lowerOrderMatrices[i];
        delete lowerOrderMatrix;
        lowerOrderMatrix=hiMs[0]->average(hiMs);
    }
    lowerOrderModel->averageLowerOrderModel(false);
}



void NthOrdRateMatrix::deleteOddOrders() {
    if(!lowerOrderModel) return;
    if(lowerOrderModel->order % 2) {
        NthOrdRateMatrix *newLowerOrderModel=lowerOrderModel->lowerOrderModel;
        lowerOrderModel->lowerOrderModel=NULL;
        delete lowerOrderModel;
        lowerOrderModel=newLowerOrderModel;
    }
    lowerOrderModel->deleteOddOrders();
}



void NthOrdRateMatrix::averageDual() {
    if(!lowerOrderModel) return;
    Array1D<RateMatrix*> &lowerOrderMatrices=lowerOrderModel->matrices;
    int halfOrder=order/2;
    int alphaRange=alphabetMap.getRangeSize();
    int numSuffixes=lowerOrderMatrices.size();
    Sequence parentSuffix, childSuffix, parentContext, childContext;
    for(int i=0 ; i<numSuffixes ; ++i) {
        parentSuffix.fromInt(i,halfOrder-1,alphabetMap);
        for(int j=0 ; j<numSuffixes ; ++j) {
            childSuffix.fromInt(j,halfOrder-1,alphabetMap);
            Array1D<RateMatrix*> hiMs(alphaRange*alphaRange);
            int nextM=0;
            for(Symbol w=0 ; w<alphaRange ; ++w) {
                Symbol x=alphabetMap.unmap(w);
                parentContext=x; parentContext.append(parentSuffix);
                for(Symbol y=0 ; y<alphaRange ; ++y) {
                    Symbol z=alphabetMap.unmap(y);
                    childContext=z; childContext.append(childSuffix);
                    hiMs[nextM++]=&lookup(parentContext,childContext);
                }
            }
            int lowerOrderMatrixIndex=
                lowerOrderModel->lookupIndex(parentSuffix,childSuffix);
            delete lowerOrderMatrices[lowerOrderMatrixIndex];
            lowerOrderMatrices[lowerOrderMatrixIndex]=hiMs[0]->average(hiMs);
        }
    }
    lowerOrderModel->averageDual();    
}



NthOrdRateMatrix *NthOrdRateMatrix::ancestralContextsOnly() {
    int acoOrder=order/2;
    NthOrdRateMatrix *newM=
        new NthOrdRateMatrix(acoOrder,seqType,matrixType,false);
    ancestralContextsOnly(*newM);
    return newM;
}


void NthOrdRateMatrix::ancestralContextsOnly(NthOrdRateMatrix &newM) {
    if(order==0) { // ZEROTH ORDER => JUST COPY
        newM.matrices[0]=matrices[0]->clone();
    }
    else if(order%2==0) { // EVEN ORDER => RECURSE TO LOWER ORDER
        lowerOrderModel->ancestralContextsOnly(newM);
    }
    else { // ODD ORDER => JUST COPY, THEN RECURSE
        int n=newM.matrices.size();
        for(int i=0 ; i<n ; ++i) {
            RateMatrix *Q=matrices[i];
            if(Q) newM.matrices[i]=Q->clone();
        }
        lowerOrderModel->ancestralContextsOnly(*newM.lowerOrderModel);
    }
}



NthOrdRateMatrix *NthOrdRateMatrix::clone() {
    NthOrdRateMatrix *M=new NthOrdRateMatrix(order,seqType,matrixType,dual);
    clone(*M);
    return M;
}



void NthOrdRateMatrix::clone(NthOrdRateMatrix &M) {
    int n=matrices.size();
    for(int i=0 ; i<n ; ++i) {
        RateMatrix *Q=matrices[i];
        M.matrices[i]=Q ? Q->clone() : NULL;
    }
    if(lowerOrderModel)
        lowerOrderModel->clone(*M.lowerOrderModel);
}



NthOrdRateMatrix *NthOrdRateMatrix::nextHigherOrder_dual() {
  // First, initialize the lower-order model within the new model, by
  // simply cloning this matrix into it
  int newOrder=order+2;
  NthOrdRateMatrix *M=new NthOrdRateMatrix(newOrder,seqType,matrixType,dual);
  NthOrdRateMatrix *Mlo=M->getLowerOrderModel(order);
  clone(*Mlo);

  // Now iterate through all contexts of the new model, and initialize each
  // by looking at a suffix (i.e., lower order)
  Sequence nmer, parentNmer, childNmer, shorterNmer;
  int numMatrices=M->matrices.size();
  for(int i=0 ; i<numMatrices ; ++i) {
    nmer.fromInt(i,newOrder,alphabetMap);
    int r=order/2;
    nmer.getSubsequence(1,r,parentNmer);
    nmer.getSubsequence(r+2,r,childNmer);
    shorterNmer=parentNmer;
    shorterNmer.append(childNmer);
    int lowerOrderIndex=shorterNmer.asInt(alphabetMap,0,order);
    RateMatrix *Q=matrices[lowerOrderIndex];
    M->matrices[i]=Q ? Q->clone() : NULL;
  }

  M->rescale();
  return M;
}



/********************************************************************
 This method returns a new model M of order N+1 in which all (N+1)-mers
 have their rate matrices initialized from the corresponding N-mer suffix
 within the current Nth-order model.  That is, this Nth-order model acts
 as a template for an (N+1)th order model, which will actually behave 
 identically to the lower order model (until perturbed, such as during
 training).  

 THIS IS USED FOR "PROGRESSIVE" TRAINING

 */
NthOrdRateMatrix *NthOrdRateMatrix::nextHigherOrder() {
  if(dual) return nextHigherOrder_dual();

  // First, initialize the lower-order model within the new model, by
  // simply cloning this matrix into it
  int newOrder=order+1;
  NthOrdRateMatrix *M=new NthOrdRateMatrix(newOrder,seqType,matrixType,dual);
  M->eqFreqs=eqFreqs;
  NthOrdRateMatrix *Mlo=M->getLowerOrderModel(order);
  clone(*Mlo);

  // Now iterate through all contexts of the new model, and initialize each
  // by looking at a suffix (i.e., lower order)
  Sequence nmer;
  int numMatrices=M->matrices.size();
  for(int i=0 ; i<numMatrices ; ++i) {
    nmer.fromInt(i,newOrder,alphabetMap);
    int lowerOrderIndex=nmer.asInt(alphabetMap,1,order);
    M->matrices[i]=matrices[lowerOrderIndex]->clone();
  }

  M->rescale();
  return M;
}



const NthOrdRateMatrix &NthOrdRateMatrix::operator=(const NthOrdRateMatrix &M)
{
  matrixType=M.matrixType;
  alphabetMap=M.alphabetMap;
  seqType=M.seqType;
  order=M.order;
  dual=M.dual;
  // ### can't copy the alphabet, because it's a reference...

  if(lowerOrderModel && M.lowerOrderModel)
    *lowerOrderModel=*M.lowerOrderModel;

  int n=matrices.size(), nM=M.matrices.size();
  if(n==nM)
    for(int i=0 ; i<n ; ++i) {
      if(matrices[i]) delete matrices[i];
      if(M.matrices[i])
	matrices[i]=M.matrices[i]->clone();
      else matrices[i]=NULL;
    }

  return *this;
}



double NthOrdRateMatrix::computeContextPrior(const Sequence &context)
{
  double prior=1.0;
  int L=context.getLength();
  for(int i=0 ; i<L ; ++i)
    prior*=eqFreqs[context[i]];
  return prior;
}



void NthOrdRateMatrix::rescale()
{
  return; // ###
  // Compute sum
  double sum=0;
  int n=matrices.size();
  Sequence context;
  int alphaRange=alphabetMap.getRangeSize();
  for(int i=0 ; i<n ; ++i) {
    RateMatrix *m=matrices[i];
    if(m) {
      context.fromInt(i,order,alphabetMap);
      double contextPrior=computeContextPrior(context);
      for(Symbol w=0 ; w<alphaRange ; ++w) {
	Symbol x=alphabetMap.unmap(w);
	double symbolPrior=eqFreqs[x];
	for(Symbol z=0 ; z<alphaRange ; ++z) {
	  Symbol y=alphabetMap.unmap(z);
	  if(y!=x)
	    sum+=(*m)(w,z)*contextPrior*symbolPrior;
	}
      }
    }
  }

  // Multiply all entries by 1/sum
  double factor=1/sum;
  for(int i=0 ; i<n ; ++i) {
    RateMatrix *m=matrices[i];
    if(m) m->rescale(factor);
  }
}



Array1D<unsigned> *NthOrdRateMatrix::getParmToContextMap()
{
  if(!parmToContextMap) {
    RateMatrix *Q=matrices[0];
    if(!Q) throw "NULL matrix in NthOrdRateMatrix::getParmToContextMap()";
    int parmsPerMatrix=Q->numParameters();
    int numMatrices=matrices.size();
    int totalParms=numMatrices*parmsPerMatrix;
    parmToContextMap=new Array1D<unsigned>(totalParms);
    Array1D<unsigned> &A=*parmToContextMap;
    unsigned contextCode=0;
    for(int parm=0 ; parm<totalParms ; ) {
      A[parm++]=contextCode;
      if(parm % parmsPerMatrix == 0) ++contextCode;
    }
  }
  return parmToContextMap;
}


