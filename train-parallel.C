...
class Application
{
  CommandLine *cmd;
  bool usingMPI;
  Array1D<MultSeqAlignment*> contextSpecificAlignments; // for TRCO only
  ...
  void loadAlignments(const String &mafFile);
  void buildContextSpecificAlignments();
public:
...
class ObjectiveFunction : public GSL::ObjectiveFunction
{
  ...
  Phylogeny &phylogeny;
  NthOrdRateMatrix &Q;
  int dimensionality, numProcesses, numBranches, numMatrixParms;
  ...
  void gradient_TRCO(const GSL::Vector &,GSL::Vector &gradient);
  double evalZerothOrder(NthOrdRateMatrix &,MultSeqAlignment &,
			 int parmIndex,double parmValue);
public:
  ObjectiveFunction(MultSeqAlignment &,Array1D<MultSeqAlignment*> 
		    &contextSpecificAlignments,Phylogeny &,NthOrdRateMatrix &,
...
int Application::main(int argc,char *argv[])
{
  ...
  cmd=new CommandLine(argc,argv,"m:tdpM:wS");
  if(cmd->numArgs()!=5)
    throw String(
"\ntrain-parallel <in.phy> <in.maf> <out.phy> <out.matrix> <config-file>\n\
\n\
options: \n\
         ...
         -w = third phase only (\"wobble\")\n\
         -S = Single CPU only (no MPI)\n\
\n"
);
  usingMPI=!cmd->option("S");
...
int Application::master(int argc,char *argv[])
{
    ...
    loadAlignments(mafFile);

    // Construct NMER table
    cout<<"MASTER constructing NMER table"<<endl;
    if(contextType==CT_NMER)
      nmerTable=new AlignmentNmerTable(*alignment,*alphabetMap,order);
...
int Application::slave(int argc,char *argv[])
{
    ...
    // Attach phylogeny to alignment
    //cout<<"SLAVE attaching phylogeny"<<endl;
	...
	case MSG_EVALUATE:
	  {
	    ...
	    if(ct==CT_NMER) {
	      NmerFelsenstein F(*phylogeny,Q,*alignment,nmerTable);
	      if(wobble)
		L=F.logLikelihood3(intervalBegin,intervalEnd);
	      else
		L=F.logLikelihood(intervalBegin,intervalEnd);
	    }
	    else {
	      NthOrdFelsenstein F(ct,*phylogeny,Q,*alignment,seqType,
				  nmerTable);
	      if(wobble)
		L=F.logLikelihood3(intervalBegin,intervalEnd);
	      else
...
{
  ObjectiveFunction f(*alignment,contextSpecificAlignments,
		      *phylogeny,*Q,gradientMethod,
...
ObjectiveFunction::ObjectiveFunction(MultSeqAlignment &alignment,
  ...
  : alignment(alignment), phylogeny(phylogeny), Q(Q),
    ...
    likelihood(NEGATIVE_INFINITY), contextSpecificAlignments(csa),
...
{
  numMatrixParms=Q.numParameters()*Q.getNumMatrices();
				   ...
				   GSL::Vector &gradient)
{
  ...
  if(contextType==CT_TRCO) { gradient_TRCO(currentPt,gradient); return; }
...
void ObjectiveFunction::fourPointSymmetric(const GSL::Vector &currentPt,
					   GSL::Vector &gradient)
{
  // Requires 4D evaluations, for dimensionality D
...
void ObjectiveFunction::gradient_TRCO(const GSL::Vector &currentPt,
					GSL::Vector &gradient)
{
  // PRECONDITION: currentPt has *not* been constrained
  ...
  const double twoDX=2*dx;
  for(int i=firstIndex ; i<numBranches ; ++i) {
    double &pi=perturbed[i];
    pi-=dx;
    double L0=f(perturbed);
    pi+=twoDX;
  ...
  NthOrdRateMatrix wrapper(0,DNA,Q.getType(),false);
  Array1D<unsigned> &parmToContext=*Q.getParmToContextMap();
  for(unsigned i=numBranches ; i<dimensionality ; ++i) {
    unsigned globalParmNum=i-numBranches;
    int localParmNum=globalParmNum % parmsPerMatrix;
    unsigned contextCode=parmToContext[globalParmNum];
    RateMatrix *M=Q.getIthMatrix(contextCode);
    MultSeqAlignment *msa=contextSpecificAlignments[contextCode];
    wrapper.replaceIthMatrix(0,M);
    double x=currentPoint[i];
    double L0=evalZerothOrder(wrapper,*msa,localParmNum,x-dx);
    double L1=evalZerothOrder(wrapper,*msa,localParmNum,x+dx);
    gradient[i]=(L1-L0)/twoDX;
  }
}



double ObjectiveFunction::evalZerothOrder(NthOrdRateMatrix &wrapper,
					  MultSeqAlignment &msa,
					  int parmIndex,
					  double parmValue)
{
  RateMatrix &M=wrapper.getIthMatrix(0);
  M.setIthParm(parmIndex,parmValue);
  M.init();
  TRCO_Felsenstein fel(0,phylogeny,wrapper,msa,DNA);
  return fel.logLikelihood();
}
...
void Application::loadAlignments(const String &mafFile)
{
  alignment=NULL;
  ...
  if(contextType==CT_MP) {
    ...
    fitch.run();
    //if(processID==0)
    //alignment->printSlice(cout,0,alignment->getLength()-1,'+',70);
  }
  phylogeny->attachAlignment(*alignment);
  alignment->setPhase(0);
  cout<<"alignment length = "<<alignment->getLength()<<endl;

  if(contextType==CT_TRCO)
    buildContextSpecificAlignments();
}



void Application::buildContextSpecificAlignments()
{
  Array1D<MultSeqAlignment*> &csa=contextSpecificAlignments;
  int numAlpha=alphabet->size();
  int numContexts=pow(float(numAlpha),order);
  csa.resize(numContexts);
  for(int nmerCode=0 ; nmerCode<numContexts  ; ++nmerCode) {
    MultSeqAlignment *msa=csa[nmerCode]=
      new MultSeqAlignment(*alphabet,(Symbol)alphabet->lookup('-'));
    msa->addGapSymbol(alphabet->lookup('.'));
    msa->addGapSymbol(alphabet->lookup('N'));
  }
  int L=alignment->getLength();
  int targetID=phylogeny->getRoot()->getID();
  AlignmentSeq &targetTrack=alignment->getIthTrack(targetID);
  const Sequence &targetSeq=targetTrack.getSeq();
  for(int i=order ; i<L ; ++i) {
    Sequence context;
    targetSeq.getSubsequence(i-order,order,context);
    unsigned nmerCode=context.asInt(*alphabetMap);
    MultSeqAlignment *A=csa[nmerCode];
    A->append(*alignment,i,1);
  }
}
