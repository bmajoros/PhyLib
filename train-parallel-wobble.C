/****************************************************************
 train-parallel-wobble.C (wobble => only uses 3rd codon positions)
 bmajoros@duke.edu
...
int Application::master(int argc,char *argv[])
{
    ...
    CommandLine cmd(argc,argv,"m:tdpM:f:");
    if(cmd.numArgs()!=5)
      throw String(
"\ntrain-parallel-wobble <in.phy> <in.maf> <out.phy> <out.matrix> <config-file>\n\
\n\
options: \n\
         ...
         -f <frame> = train only on the indicated frame (phase)\n\
\n"
);
    ...
    desiredPhase=cmd.option('f') ? cmd.optParm('f').asInt() : 2;
	...
	case MSG_EVALUATE:
	  {
	    ...
	    if(desiredPhase<0 || desiredPhase>2) throw "desiredPhase not set";
	    if(ct==CT_NMER) {
	      NmerFelsenstein F(*phylogeny,Q,*alignment,nmerTable);
	      L=F.logLikelihoodInPhase(intervalBegin,intervalEnd,desiredPhase);
	    }
	    else {
	    ...
	    //cout<<"slave evaluating columns "<<intervalBegin<<" to "<<intervalEnd<<" in phase "<<desiredPhase<<" = "<<L<<endl;
